...
import logger_setup
from models import Message, Certificate, SessionLocal
...
def inject_now() -> Dict[str, datetime]:
    """Inject the current UTC time into templates."""
...
def config() -> ResponseReturnValue:
    ...
    if request.method == "POST":
        if "cert_file" in request.files:
            ...
            config_data = {
                "iot_environment": request.form.get("iot_environment", "test"),
...
def mqtt_test() -> ResponseReturnValue:
    ...
    return cast(ResponseReturnValue, render_template("mqtt_test.html", message=message))
