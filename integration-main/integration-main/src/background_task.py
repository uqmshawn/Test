...
def init_mqtt_client(
    ...
    # Use the connect_mqtt_client function with keepalive=60
    if connect_mqtt_client(
        client, broker_address, port, max_retries, retry_delay, keepalive=60
    ):
        return client
    return None
    ...
    try:
        ...
        with SessionLocal() as session:
            new_outbound = OutboundGPSData(
                last_sent=datetime.now(timezone.utc), payload=payload_array
            )
...
def process_vehicle_outbound(client: mqtt.Client) -> None:
    ...
    # include at least the latest record
    if not payload_array and not last_outbound and vehicle_records:
        payload_array = [vdu.create_vehicle_payload(vehicle_records[-1])]
        logging.info("Including latest vehicle data as initial record")
...
def run_background_task() -> None:
    ...
    5. Obtains a router session
    6. Starts the GPS polling thread
    7. Starts the vehicle data polling thread
    8. Keeps the main thread alive
    """
    try:
        # Get MQTT broker address from environment or use default
        broker_address = os.environ.get("MQTT_BROKER_ADDRESS", "mosquitto")
        port = 1883
