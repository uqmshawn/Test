...
from timestamp_util import UTC_TIME_FORMAT, get_utc_now, format_datetime_utc
...
def store_gps_coordinates(gps: GPSCoordinates) -> None:
    ...
    now_utc = get_utc_now()  # Use our timestamp utility
...
def purge_old_messages(days: int = 14) -> None:
    ...
    logging.info(
        "Purging messages older than %d days (cutoff: %s).",
        days,
        format_datetime_utc(cutoff),
    )
...
def purge_old_gps_data(days: int = 14) -> None:
    ...
    cutoff = get_utc_now() - timedelta(days=days)
    logging.info(
        "Purging GPS data older than %d days (cutoff: %s).",
        days,
        format_datetime_utc(cutoff),
    )
...
def purge_old_device_data(days: int = 14) -> None:
    ...
    cutoff = get_utc_now() - timedelta(days=days)
    logging.info(
        "Purging device data older than %d days (cutoff: %s).",
        days,
        format_datetime_utc(cutoff),
    )
