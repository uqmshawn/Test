...
from timestamp_util import get_utc_now, get_mqtt_timestamp, ensure_mqtt_timestamp
...
def store_vehicle_data(vehicle_data: Dict[str, Any]) -> Optional[VehicleData]:
    ...
    try:
        with SessionLocal() as session:
            now = get_utc_now()  # Use our timestamp utility
...
def create_vehicle_payload(record: VehicleData) -> Dict[str, Any]:
    ...
    """
    return {
        "ts": ensure_mqtt_timestamp(record.timestamp),  # Seconds since epoch
...
def detect_vehicle_data_changes(
    ...
    # If there's no previous data, include all records
    if not last_data:
        logging.info("No previous vehicle data, including all %d records", len(records))
        return [create_vehicle_payload(record) for record in records]
